...
contract GaugeFactory is IGaugeFactory, OwnableUpgradeable {
    ...
    function initialize(VoterRolesAuthority _permissionRegistry) initializer  public {
        __Ownable_init();
        permissionsRegistry = _permissionRegistry;
        gaugeLogic = new MarketGauge();
    }

    function reinitialize() reinitializer(1) public {
        gaugeLogic = new MarketGauge();
    }
    ...
    modifier onlyAllowed() {
        require(owner() == msg.sender || permissionsRegistry.canCall(msg.sender, address(this), msg.sig), 'ERR: GAUGE_ADMIN');
        _;
    }
